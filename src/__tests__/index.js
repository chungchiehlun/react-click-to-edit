...
test("snapshot - default", () => {
  const wrapper = shallow(<ClickToEdit defaultValue="foo" />);
  expect(toJson(wrapper)).toMatchSnapshot();
});

test("snapshot - onEdit", () => {
  const wrapper = shallow(<ClickToEdit defaultValue="bar" />);
...
test("render default value normally", () => {
  const wrapper = shallow(
    <ClickToEdit defaultValue="HELLO" endEditing={jest.fn()} />
  );
...
test("invoke endEditing function and leave editing mode after pressing the enter key", () => {
  const mock = jest.fn();
  const wrapper = mount(<ClickToEdit defaultValue="HELLO" endEditing={mock} />);
  wrapper.setState({
    onEditMode: true
  });
